...
export default async function GrantedVehiclesSection(): Promise<JSX.Element | null> {
     ...
     const grantedVehicles: IDataResponse<VehicleWithTypes[]> = await getGrantedVehiclesForUser(session.user.aadObjectId!);
     // console.log("grantedVehicles: ", grantedVehicles);
     ...
     return (
          <div className="mx-2 my-2 rounded-lg bg-slate-400 px-2 py-2">
               <div className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2 md:grid-cols-3">
                    ...
                    {grantedVehicles.data?.map((v: VehicleWithTypes) => {
                         return (
                              <VehicleCard
                                   key={v._id?.toString()}
                                   vehicle={v}
